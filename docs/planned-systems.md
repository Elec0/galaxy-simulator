...
| Local player preferences | Retain presentation, accessibility, pacing, and other local settings while separating them from authoritative game state. | `TASK-050`, coordinated with `TASK-038` and `TASK-045`; [authoritative save boundary](authoritative-save-boundary.md) excludes purely local settings. |
