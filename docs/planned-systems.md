...
This document is the exhaustive system inventory produced by `TASK-044`. It is
the starting input for the scope-gap review in `TASK-043`; it is not an
...
"inventory deferral" have no approved design task yet and are deliberately
parked here for `TASK-043` to assess. That label must not be interpreted as
permission to infer a contract during implementation.

## Player application and presentation

| System | Planned responsibility | Current ownership or disposition |
| --- | --- | --- |
| Application lifecycle and shell | Start the desktop application, choose a new or saved session, leave a running session safely, and surface fatal startup failures. | Inventory deferral. `TASK-040` owns only recovery from invalid sessions or content; [Technical direction](technical-direction.md) owns the desktop boundary. |
...
| Save-slot experience and autosave | Present slots, choose autosave cadence and retention, and resolve local or cross-device file conflicts without weakening save validation. | Inventory deferral; mechanics are explicitly deferred by [save format and migration](save-format-and-migration.md#deferred-choices). |
...
| Map rendering and view hierarchy | Render galaxy topology, individual system spaces, connectors, ships, stations, motion, and useful overlays at appropriate zoom levels. | [Technical direction](technical-direction.md), [initial roadmap](roadmap.md), and [presentation snapshots](presentation-snapshots.md); broader camera and map behavior remain explicitly deferred there. |
| Input, selection, and inspection | Translate map interaction into commands; support selection, focus, entity details, relevant history, and stale-selection handling. | [Presentation snapshots](presentation-snapshots.md), completed `TASK-010`; non-ship selection is deferred and group intent belongs to `TASK-033`. |
...
| Notifications and explanation views | Present facts, command outcomes, shortages, decisions, and bounded history with useful grouping and priority. | [Semantic game facts](semantic-game-facts.md), [presentation snapshots](presentation-snapshots.md), and `TASK-025`; final notification policy is explicitly deferred. |
...
| Localization and text layout | Resolve stable presentation keys into locale-sensitive wording, formatting, pluralization, fonts, right-to-left layout, and expansion-safe UI. | `TASK-045`, explicitly required before broad player-facing game-layer work. |
| Accessibility | Define accessible input and presentation behavior without placing accessibility settings in authoritative simulation state. | `TASK-045`; detailed modes and acceptance criteria remain explicitly deferred to that task. |
...
| Local player preferences | Retain presentation, accessibility, pacing, and other local settings while separating them from authoritative game state. | [Time and pacing](time-and-pacing.md), [authoritative save boundary](authoritative-save-boundary.md), and `TASK-038`; broader settings storage is an inventory deferral. |

## Content, authoring, and compatibility

| System | Planned responsibility | Current ownership or disposition |
| --- | --- | --- |
| Content packages and manifests | Describe explicit content documents, dependencies, package identity, and optional presentation assets through one built-in and external path. | [Gameplay content](gameplay-content.md), completed `TASK-023`; implementation in `TASK-048`. |
| Declarative definitions and catalogs | Validate stable qualified identities and construct immutable catalogs for materials, designs, principals, policies, and later domain content. | [Gameplay content](gameplay-content.md); `TASK-048`. |
| Content validation and diagnostics | Headlessly parse, validate, resolve, canonicalize, fingerprint, and report content without publishing partial state. | [Gameplay content](gameplay-content.md); `TASK-048`. |
...
| Authored scenario tooling | Support rapid edit, validate, and rerun workflows for static scenarios without creating a weaker validation path. | Basic command in `TASK-048`; watch mode, editor integration, and richer tools are explicitly deferred by [gameplay content](gameplay-content.md). |
...
| Deterministic randomness | Assign stable versioned random streams whose results do not shift when unrelated systems add draws or work is repartitioned. | `TASK-021`. |
...
| Deterministic concurrency | Preserve identical results across worker counts, batch layouts, partition shapes, work stealing, and completion order. | [Concurrency and performance](concurrency-and-performance.md); later implementation and evidence in `TASK-029`. |
...
| Docking, undocking, and berths | Approach a facility, obtain access or capacity, transition between moving and docked state, and expose congestion. | Inventory deferral. [Economy](economy.md) requires docking and loading congestion, while `TASK-019` supplies the shared moving-interaction substrate. |
...
| Standing orders and recurring automation | Let the player delegate repeatable trade, mining, patrol, defense, and other routine work without creating a second command model. | Inventory deferral; explicitly deferred by [actor control and orders](actor-control-and-orders.md) and promised by [player experience](player-experience.md). |
| Autonomous ship behavior | Select ordinary ship work through the same command and order boundaries used by player intent. | Logistics foundation in completed `TASK-009` and `TASK-034`; broader policy is an inventory deferral coordinated with `TASK-026` and `TASK-042`. |
...
| Resource acquisition | Mine or collect resources with explicit deposits, capability, work, depletion or renewal policy, and resulting inventory. | Basic Phase 1 mining is implemented; general resource-world and depletion behavior are an inventory deferral owned conceptually by [economy](economy.md). |
...
| Stations and installed capabilities | Combine storage, production, trade, repair, construction, defense, and service capabilities in persistent facilities. | [Economy](economy.md); production and construction foundations exist, while generalized station composition is an inventory deferral coordinated with `TASK-023` and `TASK-041`. |
| Trade offers and contracts | Publish supply, demand, procurement, and transport opportunities for player or autonomous acceptance. | [Economy](economy.md); Phase 1 job-board foundation exists, broader contract policy is explicitly beyond Phase 1. |
| Currency, prices, and markets | Allocate scarce goods through prices while allowing internal logistics and strategic reservation outside a public market. | Explicitly deferred beyond Phase 1 by [economy](economy.md) and [Phase 1](phase-1-simulation-spec.md). |
...
| Ship acquisition and upgrades | Purchase, construct, replace, improve, and equip ships while keeping one-ship play viable. | [Player experience](player-experience.md); construction foundation exists, equipment is `TASK-041`, and purchase or upgrade policy is an inventory deferral. |
| Station construction and expansion | Build production capacity and stations from material inputs, assign ownership, and alter the regional economy. | [Vision](vision.md), [player experience](player-experience.md), and [economy](economy.md); explicitly deferred beyond current ship-construction foundation. |
| Repair and maintenance | Consume time, capacity, materials, and possibly equipment to restore damaged assets. | Inventory deferral; station capability is named by [economy](economy.md), with damage prerequisites in `TASK-046` and inventory prerequisites in `TASK-041`. |
...
| Territory and borders | Identify territorial authority, publish rules, enforce restricted space, and eventually support claims or loss of control. | Foundation in completed `TASK-012`; identities, boundaries, and claiming algorithms are explicitly deferred by [factions](factions.md). |
...
| Territory growth and player political progression | Claim space and grow from independent operator into a territorial power without making expansion mandatory. | Eventual path in [player experience](player-experience.md) and [factions](factions.md); explicitly deferred until stations and territorial conflict exist. |
...
| Dialogue and conversations | Define availability, conditions, choices, memory, consequences, continuity, and response-required pacing behavior. | `TASK-016`, coordinated with `TASK-038` and `TASK-045`. |
...
is the canonical inventory input for `TASK-043`. When a later decision adds a
planned system, removes one, or promotes an inventory deferral into owned work,
update the relevant row and add or revise the corresponding tracker task rather
than scattering a second inventory elsewhere.
