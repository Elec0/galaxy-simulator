...
| System-local movement | Plan destination intent and execute authoritative scheduled motion without making rendered interpolation authoritative. | [Navigation architecture](navigation-architecture.md), completed `TASK-005` and `TASK-028`; completed `TASK-087` defines coordinate scale and cruise transition, while `TASK-089` defines thrust, maneuver kinematics, and short moves. |
