...
| Map rendering and view hierarchy | Render galaxy topology, individual system spaces, connectors, ships, stations, motion, and useful overlays at appropriate zoom levels. | Completed `TASK-049` defines the public-static-topology and map boundary; `TASK-077` implements it, building on [technical direction](technical-direction.md) and [presentation snapshots](presentation-snapshots.md). Future nonpublic topology remains `TASK-076`. |
| Input, selection, and inspection | Translate map interaction into commands; support selection, focus, entity details, relevant history, and stale-selection handling. | Completed `TASK-010` supplies the ship-selection foundation; completed `TASK-049` defines the broader application experience; `TASK-077` implements it; and `TASK-033` owns group intent. |
...
| Notifications and explanation views | Present facts, command outcomes, shortages, decisions, and bounded history with useful grouping and priority. | [Semantic game facts](semantic-game-facts.md), `TASK-025` for retained explanation, completed `TASK-049` for the application contract, and `TASK-077` for implementation. |
...
| Accessibility | Define accessible input and presentation behavior without placing accessibility settings in authoritative simulation state. | [Internationalization and localization](internationalization-and-localization.md) establishes the text and layout baseline; completed `TASK-061` defines comprehensive modes and acceptance criteria, application implementation belongs to `TASK-077`, audio to `TASK-078` through `TASK-080`, reduced motion to `TASK-081`, and photosensitivity to `TASK-082`. |
...
| Deterministic randomness | Assign stable versioned random streams whose results do not shift when unrelated systems add draws or work is repartitioned. | Defined by [Deterministic randomness and stream ownership](deterministic-randomness.md) in completed `TASK-021`; implemented by completed `TASK-066`. |
...
| System-local movement | Plan destination intent and execute authoritative scheduled motion without making rendered interpolation authoritative. | [Navigation architecture](navigation-architecture.md), completed `TASK-005` and `TASK-028`; completed `TASK-087` defines coordinate scale and cruise transition, completed `TASK-089` defines [thrust, maneuver kinematics, and short moves](ship-maneuver-kinematics.md), and `TASK-090` retains implementation. |
...
| Sensors, detection, and scouting | Reveal current authoritative state through player-owned sensor radii, hide transient contacts outside coverage, and retain stale observations of discovered persistent structures. | Defined by [Fog-of-war and scouting](fog-of-war-and-scouting.md) in completed `TASK-020`; implementation in `TASK-073`. Stealth, occlusion, sensor sharing, equipment modifiers, moving-contact extrapolation, and unspecified discovery categories remain deferred. |
| Sensor deployables | Place persistent stationary player-owned sensor sources through an explicit entity, ownership, inventory, command, removal, and save lifecycle. | `TASK-074` defines the missing deployable entity contract before `TASK-073` consumes it. |
...
| Autonomous ship behavior | Select ordinary ship work through the same command and order boundaries used by player intent. | `TASK-053`, coordinated with `TASK-026` and `TASK-042`; completed `TASK-020` keeps player fog-of-war separate from authoritative NPC reads. |
...
| System | Planned responsibility | Current ownership or disposition |
| --- | --- | --- |
| Materials and physical inventory | Conserve typed material quantities, capacity, reservations, transfers, and destruction disposition. | [Economy](economy.md), Phase 1 foundation, completed `TASK-034`; generalized design completed by `TASK-041` with implementation in `TASK-069`. |
| General cargo and items | Model approved physical item categories, stacks or identities, cargo capacity, transfer, and save references. | [Generalized inventory and cargo](inventory-and-cargo.md), completed `TASK-041`; implementation in `TASK-069`. |
| Equipment and ship slots | Define equipment items, ship slots or hardpoints, compatibility, installation, removal, and installed capability contributions. | `TASK-068`, after generalized item and cargo semantics in `TASK-041`. |
...
| Credits, prices, and markets | Allocate scarce goods through the single unified Credits currency while allowing internal logistics and strategic reservation outside a public market. | Excluded from Phase 1; Credit balances, conservation, prices, and settlement belong to `TASK-055`. |
...
| Ship acquisition and upgrades | Purchase, construct, replace, improve, and equip ships while keeping one-ship play viable. | Construction foundation exists; the progression contract belongs to `TASK-056` after `TASK-068` and `TASK-055`. |
...
| Repair and maintenance | Consume time, capacity, materials, and possibly equipment to restore damaged assets. | `TASK-058`, after `TASK-041`, `TASK-046`, `TASK-057`, and `TASK-068`. |
...
| Player and faction knowledge | Give the player spatial fog-of-war with live sensor coverage and stale persistent discoveries without simulating general beliefs for every faction. | Defined by [Fog-of-war and scouting](fog-of-war-and-scouting.md) in completed `TASK-020`; implementation in `TASK-073`. Public relationship information retains the accepted scoped projection, while NPC knowledge simulation is deferred. |
...
| Observed and unobserved conflict | Preserve identical causal outcomes while varying player delivery according to current sensor coverage. | `TASK-046` builds on completed `TASK-020`; reduced-detail simulation remains separately evidence-gated. |
